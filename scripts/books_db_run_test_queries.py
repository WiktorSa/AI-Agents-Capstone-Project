...
def main():
    ...
    cursor = conn.execute("""SELECT
        ...
        FROM BOOKS
        WHERE UPPER(CATEGORY) LIKE UPPER('%autobiography%')
        LIMIT 3;"""
    )
    for row in cursor:
        print(row)
    ...
    cursor = conn.execute("""SELECT
        TITLE, AUTHORS, CATEGORY, DESCRIPTION
        FROM BOOKS
        WHERE UPPER(DESCRIPTION) LIKE UPPER("%corporation%") 
        LIMIT 3;"""
    )
    for row in cursor:
        print(row)
    ...
    cursor = conn.execute("""SELECT
        TITLE, AUTHORS, CATEGORY, DESCRIPTION
        FROM BOOKS
        WHERE UPPER(CATEGORY) NOT LIKE UPPER('%autobiography%')
        LIMIT 3;"""
    )
    for row in cursor:
        print(row)
    ...
    cursor = conn.execute("""SELECT
        TITLE, AUTHORS, CATEGORY, DESCRIPTION
        FROM BOOKS
        WHERE UPPER(DESCRIPTION) NOT LIKE UPPER("%corporation%") 
        LIMIT 3;"""
    )
    for row in cursor:
        print(row)
